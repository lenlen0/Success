...
class Group extends ConnexionPDO {
    ...
    public function getAllGroup() {
        $resultat = array();
        try {
            $req = $this->conn->prepare("SELECT g.idGroup, g.name, COALESCE(COUNT(bg.id_s11), 0) AS nb_user FROM `Group` g
    ...
    public function addGroup($name, $id_s11) {
        try {
            $req = $this->conn->prepare("INSERT INTO `Group` (name, id_s11)
                VALUES (:name, :id_s11)");
